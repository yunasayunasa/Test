...
// サウンドアセットのプリロード
// ---------------------------
const titleCallSound = new Audio("assets/titlecall.mp3"); // タイトルコール音
const tapSound = new Audio("assets/tapSE.mp3");            // タップ音（SE)

const gameBGM = new Audio("assets/gameBGM.m4a");           // ゲーム中BGM
gameBGM.loop = true;

const ballMergeSound = new Audio("assets/ballMergeSE.mp3");  // 旧ボール合体時SE（使用しません）

const specialAvailableSound = new Audio("assets/specialAvailableSE.mp3"); // 必殺技使用可能通知のSE

const gameOverSound = new Audio("assets/gameOverBGM.mp3");   // ゲームオーバー時の音楽

// 必殺技カットイン用ボイス（レベルに対応：cutInVoice_変数.mp3）
const cutInVoiceAssets = {};
for (let i = 1; i <= 10; i++) {
  cutInVoiceAssets[i] = new Audio(`assets/cutInVoice_${i}.mp3`);
}

// ★ 新たにレベルアップ（合体）時の音声アセットを読み込む（例：levelUpSound_1.mp3 ～ levelUpSound_10.mp3）
const levelUpSoundAssets = {};
for (let i = 1; i <= 10; i++) {
  levelUpSoundAssets[i] = new Audio(`assets/levelUpSound_${i}.mp3`);
}
...
// ボール合体SEの連打防止用（今回 levelUpSoundAssets を使用するのでこちらは参考値）
...
function playCutInEffect(level, callback) {
  ...
  cutInDiv.style.backgroundSize = "100% auto"; // 横幅に合わせる
...
const GameScreen = {
    ...
    for (let i = 0; i < pairs.length; i++) {
      ...
      if (bodyA.label === "ball" && bodyB.label === "ball" &&
        ...
        if (Date.now() - lastBallMergeTime >= ballMergeDelay) {
          // レベルアップ時の音声は、合体した結果のレベル（bodyA.ballLevel + 1、最大10）を参照
  ...
  activateSpecialMove() {
    console.log("必殺技発動！");
    // specialButtonClickHandler で既にカットインは実行済みなので、ここでは必殺技のロジックのみ
